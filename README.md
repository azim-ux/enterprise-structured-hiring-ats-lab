...
The 92-test suite and executable audit verify tracked-file policy, privacy and high-confidence secret patterns, relative links, external executable scripts, PDF text and metadata, governed artifact hashes, complete CSV contracts, every 40/40/20 composite, cohort and requisition reconciliation, both embedded dashboard payloads, the halo-control record, the five-slide structure, deterministic pagination rules, workflow pins, history privacy, redacted failure behavior, and positive and negative GitHub merge-provenance cases. A separate push-to-main CI job verifies platform-generated merge commits against GitHub's actor, signature, parent, and merged-PR evidence. See [Repository Quality Gates](QUALITY_GATES.md) and the [identity privacy and provenance method](docs/audit/IDENTITY_PRIVACY_AND_PROVENANCE_METHOD.md) for the control boundary and limitations.

The Stage 1B data gate is implemented in [`scripts/data_contracts.py`](scripts/data_contracts.py) and called by the existing `data` audit check. It compares values after string/null normalization and reports only synthetic record keys and field names. Browser interaction, responsive/accessibility behavior, continuous dependency monitoring, and production access controls remain outside this stage; those gaps are tracked in the [test-coverage map](docs/audit/TEST_COVERAGE_MAP.md) and linked Stage 1 issues.
